...
//    def on[TH <: dom.EventTarget](`type`: String, listener: ThisFunction1[TH, Datum, Unit], capture: Boolean = false): T = js.native
    ...
    def each(func: js.ThisFunction2[dom.EventTarget, Datum, Int, Unit]): T = js.native

  }

  object BaseSelectionImplicits {
    // add some convenient implicit class to convert scala functions to js.Function and js.ThisFunction
    implicit class BaseSelectionFuncArgConverter[Datum, T <: BaseSelection[Datum,T]](selection: BaseSelection[Datum, T]) {
      def attr(name: String, f: () => Primitive) = {
        val f2 = (data: Datum, idx: Int, th: js.UndefOr[Int]) => f()
        selection.attr(name, f2)
      }

      def attr(name: String, f: Datum => Primitive) = {
        val f2 = (data: Datum, idx: Int, th: js.UndefOr[Int]) => f(data)
        selection.attr(name, f2)
      }

      def attr(name: String, f: (Datum, Int) => Primitive) = {
        val f2 = (data: Datum, idx: Int, th: js.UndefOr[Int]) => f(data, idx)
        selection.attr(name, f2)
      }

      def style(name: String, f: () => Primitive) = {
        val f2 = (data: Datum, idx: Int, th: js.UndefOr[Int]) => f()
        selection.style(name, f2)
      }

      def style(name: String, f: Datum => Primitive) = {
        val f2 = (data: Datum, idx: Int, th: js.UndefOr[Int]) => f(data)
        selection.style(name, f2)
      }

      def style(name: String, f: (Datum, Int) => Primitive) = {
        val f2 = (data: Datum, idx: Int, th: js.UndefOr[Int]) => f(data, idx)
        selection.style(name, f2)
      }

      def text(f: () => Primitive) = {
        val f2 = (data: Datum, idx: Int, th: js.UndefOr[Int]) => f()
        selection.text(f2)
      }

      def text(f: Datum => Primitive) = {
        val f2 = (data: Datum, idx: Int, th: js.UndefOr[Int]) => f(data)
        selection.text(f2)
      }

      def text(f: (Datum, Int) => Primitive) = {
        val f2 = (data: Datum, idx: Int, th: js.UndefOr[Int]) => f(data, idx)
        selection.text(f2)
      }

      def on(name: String, f: () => Any) = {
        val f2: js.ThisFunction2[dom.EventTarget, Datum, Int, Unit] = (t: dom.EventTarget, data: Datum, idx: Int) => { f(); () }
        selection.on(name, f2)
      }

      def on(name: String, f: Datum => Any) = {
        val f2: js.ThisFunction2[dom.EventTarget, Datum, Int, Unit] = (t: dom.EventTarget, data: Datum, idx: Int) => { f(data); () }
        selection.on(name, f2)
      }

      def on(name: String, f: (Datum, Int) => Any) = {
        val f2: ThisFunction2[dom.EventTarget, Datum, Int, Unit] = (t: dom.EventTarget, data: Datum, idx: Int) => { f(data, idx); () }
        selection.on(name, f2)
      }

      def on(name: String, f: (dom.EventTarget, Datum, Int) => Any) = {
        val f2: ThisFunction2[dom.EventTarget, Datum, Int, Unit] = (t: dom.EventTarget, data: Datum, idx: Int) => { f(t, data, idx); () }
        selection.on(name, f2)
      }
    }

    def idDbl = (d: Double) => d: Primitive
    def idStr = (d: String) => d: Primitive
    def idBool = (d: Boolean) => d: Primitive
  }
